...
from sklearn.manifold import TSNE  # 添加t-SNE降维算法
...
def load_emotion_dicts(vocab):
    ...
    # 1. 加载普通情感词典
    try:
        with open('NTUSD_pos.txt', encoding='utf-8') as f:
    ...
    try:
        with open('NTUSD_neg.txt', encoding='utf-8') as f:
            words = set(line.strip() for line in f if line.strip())
            # 为消极词添加强度 (2.0-3.0)
            for word in words:
                emotion_intensity[word] = 2.0 + np.random.rand()  # 2.0-3.0
...
# 构建BERT+MLP+情感增强模型
def build_bert_mlp_model(max_len, emo_dim, dropout_rate=0.2):
    """构建BERT+MLP情感分析模型 - 结合情感词典增强"""
    # 文本输入
    ...
    try:
        ...
        cls_token = bert_output.last_hidden_state[:, 0, :]  # 使用CLS token作为句子表示
        ...
        # ===== 门控注意力融合机制 =====
        # 1. 将BERT特征投影到384维
        bert_proj = Dense(384, activation='relu', name="bert_proj")(cls_token)
        bert_proj = LayerNormalization(name="bert_norm")(bert_proj)

        # 2. 门控机制 - 基于BERT特征生成门控信号
        gate = Dense(384, activation='sigmoid', name="fusion_gate")(bert_proj)

        # 3. 应用门控
        gated_emotion = Multiply(name="gate_emotion")([emotion_features, gate])

        # 4. 残差连接
        combined = Add(name="fusion_add")([bert_proj, gated_emotion])
        combined = LayerNormalization(name="fusion_combined_norm")(combined)

        # ===== MLP多层感知机增强特征融合 =====
        x = Dense(512, activation='relu',
                  kernel_initializer=Orthogonal(),
                  kernel_regularizer=l1_l2(l1=1e-4, l2=1e-4),
                  name="mlp_dense1")(combined)
        x = LayerNormalization(name="mlp_norm1")(x)
        x = Dropout(dropout_rate, name="mlp_dropout1")(x)
        ...
        x = Dense(512, activation='relu',
                  ...
                  kernel_regularizer=l1_l2(l1=1e-4, l2=1e-4),
                  name="mlp_dense2")(x)
        x = LayerNormalization(name="mlp_norm2")(x)
        x = Dropout(dropout_rate, name="mlp_dropout2")(x)

        # 添加残差连接
        x = Add(name="mlp_residual_add")([residual, x])
        x = LayerNormalization(name="mlp_residual_norm")(x)
        x = Activation('relu', name="mlp_residual_activation")(x)

        # 降维层 - 我们稍后将使用这一层的输出进行t-SNE可视化
        feature_layer = Dense(256, activation='relu',
                              kernel_initializer=Orthogonal(),
                              kernel_regularizer=l1_l2(l1=1e-4, l2=1e-4),
                              name="mlp_dense3")(x)
        feature_layer = LayerNormalization(name="mlp_norm3")(feature_layer)
        feature_layer = Dropout(0.1, name="mlp_dropout3")(feature_layer)

        # 输出层
        output = Dense(1, activation='sigmoid', dtype='float32', name="output")(feature_layer)

        # 构建模型
        model = Model(
            inputs=[input_ids, attention_mask, emotion_input],
            outputs=output,
            name="BERT_MLP_PoliceEmotion"
        )
        ...
        feature_extractor = Model(
            inputs=model.input,
            outputs=feature_layer  # 输出降维层的特征
        )
        return model, feature_extractor, 'bert-mlp-enhanced'
...
if __name__ == "__main__":
    ...
    # 构建高性能模型
    print("构建BERT+MLP公安信访情感分析模型...")
    model, feature_extractor, model_type = build_bert_mlp_model(
        max_len,
    ...
    # 1. 提取模型的高维特征表示（分类层之前的特征）
    print("提取测试集样本的特征向量...")
    embeddings = feature_extractor.predict(test_inputs, batch_size=batch_size)

    # 2. 随机采样1000个样本进行可视化（避免计算量过大）
    n_samples = min(1000, len(embeddings))
    sample_indices = np.random.choice(len(embeddings), n_samples, replace=False)

    # 3. 使用plot_tsne函数进行可视化
    plot_tsne(
        embeddings[sample_indices],
        y_test[sample_indices],
        title=f"BERT+MLP公安信访情感分析特征空间 (n={n_samples})",
        save_path="bert_mlp_tsne.png"
    )
