...
def extract_emotional_features(words, positive_words, negative_words, emotion_intensity):
    """提取关键情感特征 - 增强版"""
    if not words:
        return np.zeros(10, dtype=np.float32)  # 特征维度增加到10
    ...
    return np.array([
        ...
        exclamation,
        question,
        negation_count  # 新增否定词计数
    ], dtype=np.float32)


def build_police_bert_model(max_len, emo_dim, dropout_rate=0.2):
    """构建纯BERT模型 - 公安信访优化版"""
    # 文本输入
    ...
    try:
        ...
        cls_token = bert_output.last_hidden_state[:, 0, :]  # 使用CLS token
        sequence_output = bert_output.last_hidden_state  # 整个序列输出
        ...
        # ===== 简单融合机制 =====
        # 1. 将BERT的CLS token投影到256维
        bert_proj = Dense(256, activation='relu',
                          kernel_initializer='glorot_uniform',
                          name="bert_proj")(cls_token)
        bert_proj = LayerNormalization(name="bert_norm")(bert_proj)

        # 2. 门控机制 - 基于BERT特征生成门控信号
        gate = Dense(256, activation='sigmoid', name="fusion_gate")(bert_proj)
        ...
        x = LayerNormalization(name="classifier_norm2")(x)
        x = Dropout(0.1, name="classifier_dropout2")(x)

        # 输出层 - 使用float32精度确保数值稳定性
        ...
        # 构建特征提取模型 (输出BERT的CLS token)
        feature_extractor = Model(
            inputs=[input_ids, attention_mask, emotion_input],
            outputs=cls_token,  # 输出CLS token (768维)
        ...
        for layer in bert_model.layers:
            if layer.name == "bert":
                for i, inner_layer in enumerate(layer.encoder.layer):
                    inner_layer.trainable = (i >= 4)  # 只训练最后8层
        ...
        return train_model, feature_extractor, 'bert-police'
...
# t-SNE可视化函数
def plot_tsne(embeddings, labels, title="BERT特征空间可视化", save_path="bert_tsne.png"):
    """
...
if __name__ == "__main__":
    ...
    print(f"总积极情感词数量: {len(positive_words)}, 总消极情感极数量: {len(negative_words)}")
    ...
    # 构建高性能模型
    print("构建公安信访BERT模型...")
    model, feature_extractor, model_type = build_police_bert_model(
        max_len,
    ...
    plot_tsne(
        ...
        title=f"BERT公安信访情感分析特征空间 (n={n_samples})",
        save_path="bert_tsne.png"
    )

    # 保存模型
    model_save_name = f'police_bert_model.keras'
    print(f"\n保存高性能模型: {model_save_name}")
    model.save(model_save_name, save_format='tf')

    # 保存性能报告
    report = f"""===== 公安信访BERT模型报告 =====
...
- 特征可视化: bert_tsne.png
"""
    with open('police_bert_report.txt', 'w', encoding='utf-8') as f:
        f.write(report)

    print(f"\n总耗时: {time.time() - start_time:.2f}秒")
    print("公安信访BERT模型训练完成")
